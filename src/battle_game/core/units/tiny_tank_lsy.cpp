...
Tank_lsy::Tank_lsy(GameCore *game_core, uint32_t id, uint32_t player_id)
...
void Tank_lsy::Update() {
  TankMove(3.0f, glm::radians(180.0f));
  TurretRotate();
  Fire();
}

void Tank_lsy::TankMove(float move_speed, float rotate_angular_speed) {
  ...
  if (fire_count_down_ == 0) {
    ...
    if (player) {
      ...
      if (input_data.mouse_button_down[GLFW_MOUSE_BUTTON_LEFT]) {
        ...
        GenerateBullet<bullet::BounceBall>(
...
bool Tank_lsy::IsHit(glm::vec2 position) const {
...
const char *Tank_lsy::UnitName() const {
  return "Tiny Tank_lsy";
}

const char *Tank_lsy::Author() const {
  return "Sigma166";
}
}  // namespace battle_game::unit
